...
import { benchEngineReady, resolveNemotronModelDir } from "./engines"
        ...
        it(`scores ${set.id} (${fixtures.length} fixtures)`, async () => {
            ...
            for (const fixture of fixtures) {
                const result = await runFixture({
                    ...
                    variant: { id: "nemotron/batch", engine: "nemotron", decode: "batch" },
                    mode: "max"
                })
                const metrics = scoreRun(result, fixture.transcript)
                const lag = metrics.commitLag!

                console.log(`\n=== ${fixture.id} (${(result.audioDurationMs / 1000).toFixed(1)}s ${fixture.tier}) ===`)
                console.log(`  startup       ${result.startupMs} ms`)
                console.log(`  decode cost   ${metrics.decodeCostRatio.toFixed(3)} x realtime   (pushBlocked p99 ${result.pacer.pushBlockedP99} max ${result.pacer.pushBlockedMax} ms)`)
                if (metrics.wer) console.log(`  WER           ${(metrics.wer.wer * 100).toFixed(1)}%  (S ${metrics.wer.substitutions} D ${metrics.wer.deletions} I ${metrics.wer.insertions} / N ${metrics.wer.refLength})`)
                console.log(`  commit lag    mean ${Math.round(lag.lag.mean)} p50 ${lag.lag.p50} p90 ${lag.lag.p90} max ${lag.lag.max} ms over ${lag.words} words (${lag.neverInterim} with no interim lead)`)
                console.log(`  interim echo  ${metrics.interimEchoes.length}`)
